...
(function() {
    ...
    if (!window.mobilePerf || !window.mobilePerf.isMobile) return;
    
    console.log('🚀 Applying mobile performance patches to script.js');
    ...
    // Override expensive querySelectorAll operations
    ...
    document.querySelectorAll = function(selector) {
        ...
        // For frequently used selectors, use cache
        const frequentSelectors = [
            '.size-options button, .desktop-size-btn',
            '.mobile-size-btn',
            '.mobile-color-btn', 
            '.mobile-texture-btn',
            '.touch-active'
        ...
        if (frequentSelectors.includes(selector)) {
    ...
    function debounceFrameUpdate() {
        let timeout;
        const originalUpdateFrameSize = window.updateFrameSize;
        
        if (typeof originalUpdateFrameSize === 'function') {
            window.updateFrameSize = function() {
                clearTimeout(timeout);
                timeout = setTimeout(() => {
                    originalUpdateFrameSize.apply(this, arguments);
                }, 50); // Debounce by 50ms
            };
        }
    }
    
    // Optimize animation cleanup
    function optimizeAnimationCleanup() {
        const originalSetTimeout = window.setTimeout;
        
        // Track animation timeouts to avoid memory leaks
        const animationTimeouts = new Set();
        
        window.setTimeout = function(callback, delay) {
            if (delay >= 600 && delay <= 1000) { // Animation-related timeouts
                const timeoutId = originalSetTimeout(() => {
                    animationTimeouts.delete(timeoutId);
                    callback();
                }, Math.min(delay, 300)); // Reduce animation delays on mobile
    ...
    function optimizeSliderUpdates() {
        const sliderUpdateHandler = window.mobilePerf.throttle((e) => {
            ...
            window.requestAnimationFrame(() => {
                const type = id.replace('mobile', '').toLowerCase();
                if (state.adjustments && type in state.adjustments) {
                    state.adjustments[type] = value;
                    updateImageFilters();
                }
            });
        }, 32); // ~30fps for smooth updates
        
        // Replace existing slider handlers
        document.addEventListener('input', (e) => {
            if (e.target.type === 'range') {
                sliderUpdateHandler(e);
            }
        }, { passive: true });
    }
    
    // Reduce DOM queries in button operations
    function optimizeButtonOperations() {
        // Cache button groups
        const buttonGroups = {
            size: null,
            color: null,
        ...
        function getButtonGroup(type) {
            if (!buttonGroups[type]) {
                switch(type) {
                    case 'size':
                        buttonGroups[type] = document.querySelectorAll('.size-options button, .desktop-size-btn, .mobile-size-btn');
                        break;
                    case 'color':
                        buttonGroups[type] = document.querySelectorAll('.color-options button, .mobile-color-btn');
                        break;
                    case 'texture':
                        buttonGroups[type] = document.querySelectorAll('.texture-options button, .mobile-texture-btn');
                        break;
                }
            }
            return buttonGroups[type];
        }
        
        // Expose optimized button group getter
        window.getOptimizedButtonGroup = getButtonGroup;
        
        // Clear cache when needed
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => {
                Object.keys(buttonGroups).forEach(key => {
                    buttonGroups[key] = null;
                });
            }, 1000);
        });
    }
    ...
    setTimeout(() => {
        debounceFrameUpdate();
        optimizeAnimationCleanup();
        optimizeSliderUpdates();
        optimizeButtonOperations();
        
        console.log('✅ Mobile performance patches applied successfully');
        console.log(`📊 DOM queries tracked: ${queryCount}`);
    }, 100);
