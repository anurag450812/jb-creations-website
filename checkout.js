...
const checkoutSavedAddressState = {
    addresses: []
};

function normalizeCheckoutSavedAddressValue(value) {
    if (value === undefined || value === null) return '';
    return String(value).trim().replace(/\s+/g, ' ');
}

function buildCheckoutSavedAddressText(address) {
    const street = normalizeCheckoutSavedAddressValue(address.street);
    const city = normalizeCheckoutSavedAddressValue(address.city);
    const state = normalizeCheckoutSavedAddressValue(address.state);
    const landmark = normalizeCheckoutSavedAddressValue(address.landmark);
    const pincode = normalizeCheckoutSavedAddressValue(address.pincode);
...
function sanitizeCheckoutSavedAddresses(addresses = []) {
    if (!Array.isArray(addresses)) return [];

    const sanitizedAddresses = [];

    addresses.forEach((address, index) => {
        const street = normalizeCheckoutSavedAddressValue(address && address.street);
        const city = normalizeCheckoutSavedAddressValue(address && address.city);
        const state = normalizeCheckoutSavedAddressValue(address && address.state);
        const landmark = normalizeCheckoutSavedAddressValue(address && address.landmark);
        const pincode = normalizeCheckoutSavedAddressValue(address && address.pincode);
        ...
        const normalizedAddress = {
            id: (address && address.id) || `saved-address-${index + 1}`,
            street,
            city,
            state,
            landmark,
            pincode,
            fullAddress: normalizeCheckoutSavedAddressValue(address && address.fullAddress) || buildCheckoutSavedAddressText({
                street,
                city,
                state,
                landmark,
        ...
        const exists = sanitizedAddresses.some((item) => {
            return item.street === normalizedAddress.street &&
                item.city === normalizedAddress.city &&
                item.state === normalizedAddress.state &&
                item.landmark === normalizedAddress.landmark &&
                item.pincode === normalizedAddress.pincode;
        });

        if (!exists && sanitizedAddresses.length < 3) {
            sanitizedAddresses.push(normalizedAddress);
        }
    });
...
function renderCheckoutSavedAddresses(addresses = []) {
    const container = document.getElementById('savedAddressSuggestions');
    const list = document.getElementById('savedAddressList');

    if (!container || !list) return;

    checkoutSavedAddressState.addresses = sanitizeCheckoutSavedAddresses(addresses);
    list.innerHTML = '';

    if (checkoutSavedAddressState.addresses.length === 0) {
        container.style.display = 'none';
        return;
    }

    checkoutSavedAddressState.addresses.forEach((address, index) => {
        const card = document.createElement('div');
        card.className = 'saved-address-card';

        const cardHead = document.createElement('div');
        cardHead.className = 'saved-address-card-head';

        const title = document.createElement('div');
        title.className = 'saved-address-title';
        title.textContent = `Saved Address ${index + 1}`;
        cardHead.appendChild(title);

        if (address.pincode) {
            const meta = document.createElement('div');
            meta.className = 'saved-address-meta';
            meta.textContent = `PIN ${address.pincode}`;
            cardHead.appendChild(meta);
        }

        const text = document.createElement('div');
        text.className = 'saved-address-text';
        text.textContent = address.fullAddress || buildCheckoutSavedAddressText(address);

        const actionButton = document.createElement('button');
        actionButton.type = 'button';
        actionButton.className = 'saved-address-action';
        actionButton.dataset.addressId = address.id;
        actionButton.textContent = 'Use this address';
    ...
    if (list.dataset.savedAddressBound !== 'true') {
        list.dataset.savedAddressBound = 'true';
        list.addEventListener('click', function(event) {
            const button = event.target.closest('.saved-address-action');
            if (!button) return;

            applySavedAddressToCheckout(button.dataset.addressId);
        });
    }
...
function updateCheckoutFieldAfterSavedAddress(fieldId, value, required = false) {
...
function applySavedAddressToCheckout(addressId) {
    const address = checkoutSavedAddressState.addresses.find((item) => item.id === addressId);
    if (!address) return;

    updateCheckoutFieldAfterSavedAddress('customerAddress', address.street, true);
    updateCheckoutFieldAfterSavedAddress('customerCity', address.city, true);
    updateCheckoutFieldAfterSavedAddress('customerState', address.state, true);
    updateCheckoutFieldAfterSavedAddress('customerLandmark', address.landmark || '');
    updateCheckoutFieldAfterSavedAddress('customerPincode', address.pincode || '');
    ...
    if (typeof notifications !== 'undefined' && notifications.info) {
        notifications.info('Saved address applied. You can still edit it before placing the order.');
    }
}

async function waitForCheckoutApi(maxAttempts = 8) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const api = window.jbAPI || window.jbApi;
        if (api && typeof api.getUserByPhone === 'function') {
            return api;
        }
...
async function loadSavedAddressesForCheckout() {
    ...
    try {
        let savedAddresses = sanitizeCheckoutSavedAddresses(user.savedAddresses || []);
        const api = await waitForCheckoutApi();

        if (api && typeof api.getUserByPhone === 'function') {
            const result = await api.getUserByPhone(user.phone);
            if (result && result.success && result.user) {
                savedAddresses = sanitizeCheckoutSavedAddresses(result.user.savedAddresses || savedAddresses);
            }
        }

        renderCheckoutSavedAddresses(savedAddresses);
    } catch (error) {
        console.warn('⚠️ Unable to load saved addresses for checkout:', error);
        renderCheckoutSavedAddresses([]);
    }
}

function buildSavedAddressFromOrderData(orderData) {
    const addressDetails = orderData && orderData.customer ? orderData.customer.addressDetails : null;
    if (!addressDetails) return null;

    const street = normalizeCheckoutSavedAddressValue(addressDetails.street);
    const city = normalizeCheckoutSavedAddressValue(addressDetails.city);
    const state = normalizeCheckoutSavedAddressValue(addressDetails.state);
    const landmark = normalizeCheckoutSavedAddressValue(addressDetails.landmark);
    const pincode = normalizeCheckoutSavedAddressValue(addressDetails.pincode);
    ...
    return {
        ...
        landmark,
        pincode,
        fullAddress: buildCheckoutSavedAddressText({
            street,
            city,
            state,
            landmark,
...
async function saveCheckoutAddressToProfile(orderData, user) {
    const currentUser = user || getCurrentUser();
    if (!currentUser || !currentUser.phone) {
        return;
    }

    const addressPayload = buildSavedAddressFromOrderData(orderData);
    if (!addressPayload) {
        return;
    }
    ...
    try {
        const api = await waitForCheckoutApi();
        if (!api || typeof api.addUserSavedAddress !== 'function') {
            return;
        }

        const result = await api.addUserSavedAddress(currentUser.phone, addressPayload, {
            name: orderData.customer?.name || currentUser.name || 'User',
            email: orderData.customer?.email || currentUser.email || '',
            phone: currentUser.phone,
            signInMethod: currentUser.signInMethod || 'phone'
        });

        if (result && result.success && result.saved) {
            console.log('🏠 Checkout address saved to user profile');
        } else if (result && result.duplicate) {
            console.log('🏠 Checkout address already exists in saved addresses');
        } else if (result && result.limitReached) {
            console.log('🏠 Saved address limit reached, auto-save skipped');
        }
    } catch (error) {
        console.warn('⚠️ Failed to save checkout address to profile:', error);
    }
}
        ...
        if (paymentResult.success) {
            ...
            if (result.success) {
                ...
                if (result.method === 'firebase') {
                    ...
                    await saveCheckoutAddressToProfile(orderData, user);
