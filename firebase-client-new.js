...
class JBCreationsAPI {
    ...
    normalizeSavedAddressValue(value) {
        if (value === undefined || value === null) return '';
        return String(value).trim().replace(/\s+/g, ' ');
    }

    buildSavedAddressRecord(addressData = {}) {
        const street = this.normalizeSavedAddressValue(addressData.street);
        const city = this.normalizeSavedAddressValue(addressData.city);
        const state = this.normalizeSavedAddressValue(addressData.state);
        const landmark = this.normalizeSavedAddressValue(addressData.landmark);
        const pincode = this.normalizeSavedAddressValue(addressData.pincode);
        ...
        return {
            id: addressData.id || `addr_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
            street,
            city,
            state,
            landmark,
    ...
    isSameSavedAddress(firstAddress, secondAddress) {
        if (!firstAddress || !secondAddress) return false;

        return ['street', 'city', 'state', 'landmark', 'pincode'].every((key) => {
            return this.normalizeSavedAddressValue(firstAddress[key]) === this.normalizeSavedAddressValue(secondAddress[key]);
        });
    }

    getSanitizedSavedAddresses(addresses = []) {
        ...
        addresses.forEach((address) => {
            try {
                const normalizedAddress = this.buildSavedAddressRecord(address);
                const alreadyExists = sanitizedAddresses.some((item) => this.isSameSavedAddress(item, normalizedAddress));

                if (!alreadyExists && sanitizedAddresses.length < 3) {
                    sanitizedAddresses.push(normalizedAddress);
                }
            } catch (error) {
                console.warn('⚠️ Skipping invalid saved address:', error.message);
            }
        });
    ...
    async addUserSavedAddress(phoneNumber, addressData, fallbackUserData = null) {
        try {
            console.log('🏠 Adding saved address for user:', phoneNumber);

            let userResult = await this.getUserByPhone(phoneNumber);

            if ((!userResult || !userResult.success || !userResult.user) && fallbackUserData) {
                const createResult = await this.createUser({
                    name: fallbackUserData.name || 'User',
                    email: fallbackUserData.email || '',
                    phone: fallbackUserData.phone || phoneNumber,
                    signInMethod: fallbackUserData.signInMethod || 'phone'
                });

                if (!createResult.success) {
                    return { success: false, error: createResult.error || 'Failed to create user profile' };
                }

                userResult = await this.getUserByPhone(phoneNumber);
            }
            ...
            const nextAddress = this.buildSavedAddressRecord(addressData);
            const savedAddresses = this.getSanitizedSavedAddresses(userResult.user.savedAddresses || []);
            const duplicateAddress = savedAddresses.find((address) => this.isSameSavedAddress(address, nextAddress));
            ...
            const updatedAddresses = [...savedAddresses, nextAddress];

            await this.db.collection('users').doc(userResult.user.docId).update({
    ...
    async deleteUserSavedAddress(phoneNumber, addressId) {
        try {
            console.log('🗑️ Deleting saved address for user:', phoneNumber);

            const userResult = await this.getUserByPhone(phoneNumber);
